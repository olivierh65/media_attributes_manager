...
use Drupal\media_attributes_manager\CustomFieldsTraitHelper;
...
class BulkEditModalController extends ControllerBase {
  ...
  public function modal(Request $request) {
    ...
    // Titre de la modale.
          $this->t('Bulk Edit Media Attributes'),
  ...
  public function submitForm(Request $request) {
    ...
    try {
      ...
      if ($processed_count > 0) {
        ...
        $custom_fields_helper = new CustomFieldsTraitHelper();
  ...
  protected function updateMediaEntities(array $fields, array $media_ids, $media_bundle = NULL) {
    ...
    foreach ($fields as $key => $value) {
      ...
      elseif (str_ends_with($key, '_clear')) {
    ...
    try {
      ...
      foreach ($media_ids as $media_id) {
        ...
        foreach ($fields_to_update as $field_name => $value) {
          ...
          $clear_value = $fields[$clear_field_name] ?? FALSE;
